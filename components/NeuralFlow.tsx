...
const NeuralFlow = () => {
  ...
  useEffect(() => {
    ...
    class Particle {
      ...
      draw() {
        if (this.trailX.length > 1) {
          ...
          this.ctx.globalAlpha = (this.life / this.maxLife) * 0.2;

          this.ctx.moveTo(this.trailX[0], this.trailY[0]);
          for (let i = 1; i < this.trailX.length; i++) {
            this.ctx.lineTo(this.trailX[i], this.trailY[i]);
          }
          this.ctx.stroke();
        }
        ...
        this.ctx.globalAlpha = (this.life / this.maxLife) * 0.5;
    ...
    const init = () => {
      resize();
      particles = Array.from({ length: 2000 }, () => new Particle(canvas, ctx));
    };
